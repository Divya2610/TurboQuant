#!/usr/bin/env python3
"""
benchmark_script_gemma2b_gpu.py — Gemma 4 E2B · TurboQuant KV Cache · GPU
===========================================================================
...
class VRAMSampler:
    ...
    def start(self):
        # set_device() forces CUDA context creation so reset_peak_memory_stats
        # never receives an uninitialised device — this is the fix for
...
class TurboQuantEngine:
    ...
    def estimate_sizes_mb(self, context_len: int) -> dict:
        h, d, L, seq = self.num_kv_heads, self.head_dim, self.num_layers, context_len
        float16_bytes = seq * h * d * 2 * 2 * L
        k_idx_bytes   = seq * h * d * (self.key_bits / 8) * L
        v_idx_bytes   = seq * h * d * (self.val_bits / 8) * L
        scale_bytes   = seq * h * 1 * 2 * 2 * L
...
def print_header(args, device, tq_engine: TurboQuantEngine):
    print(SEP2)
    print("  Gemma 4 E2B  ·  TurboQuant KV Cache  ·  GPU Benchmark")
    print(SEP2)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Gemma 4 E2B with TurboQuant KV cache on GPU."
    )
    ...
    # Initialise CUDA context BEFORE constructing VRAMSampler.
    # Without this, reset_peak_memory_stats() receives an uninitialised device
    # and raises "RuntimeError: Invalid device argument".
    ...
    tq_engine = TurboQuantEngine(
        model, args.key_bits, args.val_bits, device, dtype=torch.float16
    )
...
# #!/usr/bin/env python3
# """
# benchmark.py — Gemma 4 E2B · TurboQuant KV Cache · GPU
# =======================================================
# Metrics  :  TTFT  |  Latency  |  Throughput  |  RAM (GB)  |  VRAM (GB)
# Context  :  2 048 tokens (fixed)
# Backend  :  CUDA GPU, float16
