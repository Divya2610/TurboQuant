#!/usr/bin/env python3
"""
benchmark.py — Gemma 4 E4B · TurboQuant KV Cache · CPU-only
=============================================================
...
Fixes applied vs original:
  ...
  2. Hardcoded layer count (42) removed from print_summary — now reads
     num_layers, num_kv_heads, and head_dim from tq_engine directly so
     the printed sizes always match what the engine actually compressed.
  3. uint8 overflow guard added — bucketize indices are stored as uint8
     which silently overflows above 8 bits (> 255 levels).
  4. Codebook is fitted on synthetic Gaussian data (not real KV tensors).
     ...
     real data would give a tighter codebook. Documented clearly.
"""
...
class LayerCompressor:
    ...
    def _compress(self, x, Q, bounds):
        ...
        # FIX 3: indices stored as uint8 (0–255).
        # Safe for up to 8 bits (256 levels). The bit-width guard in
...
class TurboQuantEngine:
    def __init__(self, model, key_bits: int, val_bits: int):
        ...
        # ── FIX 2: read all layer geometry from config dynamically ────────────
        # Previously num_layers was hardcoded as 42 in print_summary.
        # Now we read it here and expose it as an attribute so print_summary
        # always stays in sync with what the engine actually compressed,
        # regardless of which model directory is passed via --model.
        ...
        # ── FIX 1: store num_kv_heads for correct MB formula ─────────────────
        # KV projections use num_key_value_heads (GQA/MQA), not
        # num_attention_heads. Without this, displayed MB figures are off by
        # (num_kv_heads × head_dim) — typically thousands of times too small.
        # The compression ratio was unaffected because both sides of the
        # division were missing the same factor and it cancelled out.
        ...
        print(
            f"  [TurboQuant] {self.num_layers} layer compressors ready "
...
def print_header(args):
    print(SEP2)
    print("  Gemma 4 E4B  ·  TurboQuant KV Cache  ·  CPU-only Benchmark")
    print(SEP2)
...
def print_summary(results, peak_ram, args, tq_engine):
    """
    Print benchmark summary table and KV cache compression stats.

    tq_engine is passed in so that num_layers, num_kv_heads, and head_dim
    are always read from the same source that ran the actual compression —
    removing the fragile hardcoded 42 that existed in the original.
    """
    ...
    # ── FIX 1 + FIX 2: correct, dynamic KV cache size formula ────────────────
    #
    # Full formula:
    #   float32 : tokens × 2(K+V) × layers × num_kv_heads × head_dim × 4 bytes
    #   turbo   : tokens × layers × num_kv_heads × head_dim × (key_bits+val_bits) / 8 bytes
    #             (the ×2 for K and V is already embedded in (key_bits + val_bits))
    #
    # Original had two problems:
    #   - Missing num_kv_heads × head_dim  → MB figures thousands of times too small
    #   - Hardcoded 42 layers              → silently wrong if model has a different count
    #
    # Both are now fixed by reading values from tq_engine, which in turn read
    # them from the live model config at initialization time.
    ...
    print()
    print(
        f"  TURBOQUANT KV CACHE COMPRESSION\n"
        f"  (context={CONTEXT_WINDOW} tokens · {num_layers} layers · "
...
def main():
    ...
    # FIX 3: TurboQuantEngine now validates key_bits / val_bits <= 8 on init
    ...
    # FIX 1 + 2: pass tq_engine into print_summary so it can read
    # num_kv_heads, head_dim, and num_layers dynamically from the engine
    # instead of using the hardcoded 42 that was in the original.
    print_summary(results, peak_total, args, tq_engine)

    csv_path = "benchmark_results_gemma4_e4b.csv"
    with open(csv_path, "w") as f:
        ...
        # also log the model geometry so the CSV is self-documenting
        f.write(f"num_layers,{tq_engine.num_layers}\n")
        f.write(f"num_kv_heads,{tq_engine.num_kv_heads}\n")
        f.write(f"head_dim,{tq_engine.head_dim}\n")
        f.write(f"key_bits,{args.key_bits}\n")
        f.write(f"val_bits,{args.val_bits}\n")
...
#     csv_path = "benchmark_results_gemma4.csv"
#     with open(csv_path, "w") as f:
#         f.write("run,ttft_s,latency_s,throughput_tps,tokens_generated\n")
#         for i, r in enumerate(results, 1):
